<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('home')" :active="request()->routeIs('home', 'scholarships.show')">
                {{ __('Scholarships') }}
            </x-responsive-nav-link>

            @role('student')
                <x-responsive-nav-link :href="route('student.dashboard')" :active="request()->routeIs('student.dashboard', 'student.applications.*')">
                    {{ __('My Applications') }}
                </x-responsive-nav-link>
            @endrole

            @role('reviewer')
                <x-responsive-nav-link :href="route('reviewer.dashboard')" :active="request()->routeIs('reviewer.dashboard', 'reviewer.applications.*')">
                    {{ __('Review Applications') }}
                </x-responsive-nav-link>
            @endrole

            @role('coordinator')
                <x-responsive-nav-link :href="route('coordinator.dashboard')" :active="request()->routeIs('coordinator.dashboard', 'coordinator.programs.*')">
                    {{ __('Scholarships Management') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('coordinator.applications.index')" :active="request()->routeIs('coordinator.applications.*')">
                    {{ __('Applications') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('coordinator.users.index')" :active="request()->routeIs('coordinator.users.*')">
                    {{ __('Manage Users') }}
                </x-responsive-nav-link>
            @endrole
        </div>
